...
class RateLimiter:
    ...
    def check_rate_limit(self, user_id: str) -> dict:
        ...
        if user_id in self.last_reset:
            ...
            recent_request = [ts for ts in self.request[user_id] if ts > one_minute_ago]

            if len(recent_request) >= 30:
                return {
                    "allowed": False,
            ...
            self.requests[user_id] = recent_request + [now]
...
def validate_input_length(text: str, max_length: int = 200) -> dict:
    ...
    for pattern in JAILBREAK_PATTERNS:
        if re.search(pattern, text_lower):
            ...
            return {
                ...
                "pattern": pattern,
                "severity": "high",
                "response": "Hmm, kayaknya kamu coba sesuatu yang nggak biasa nih. Aku di sini untuk bantu hal-hal seputar rumah sakit aja ya. Ada yang bisa aku bantu atau mau coba ganti topik lain?"
            }
        
        return{"detected": False, "pattern": None, "severity": "none"}

HARMFUL_KEYWORDS = {
    "violence": ["bunuh", "membunuh", "tikam", "meledak", "ledak", "bom", "teror"],
    "sexual": ["seks", "telanjang", "porno", "ngentot", "kontol", "memek", "pler", "titit", "ngewe"],
    "hate_speech": ["anjing", "babi", "monyet", "kafir", "bego", "tolol", "goblok"],
    "illegal": ["narkoba", "heroin", "sabu", "ganja", "fentanyl", "kokain", "cocaine", "weed", "drug", "ekstasi", "ecstacy"],
    "self_harm": ["bunuh diri", "bundir", "suicide", "mau mati", "pengen mati", "nyayat", "gantung diri", "minum racun", "unalive myself"]
}

MEDICAL_SENSITIVE = [
    "aborsi", "bunuh diri", "overdosis", "kecanduan", "depresi berat", "skizofrenia", "psikosis"
]

def moderate_content(text: str) -> dict:
    text_lower = text.lower()

    for category, keyword in HARMFUL_KEYWORDS.items():
        if keyword in text_lower:
            ...
            if category == "self_harm":
                return {
                    "safe": False,
                    "category": category,
                    "response": ("Aku khawatir dengan apa yang kamu rasakan."
                                 "Kalau kamu butuh bantuan, silahkan hubungi:\n\n"
        ...
        for topic in MEDICAL_SENSITIVE:
            if topic in text_lower:
                ...
                return {
                    ...
                    "disclaimer": "\n\n ** Disclaimer **: Aku bukan profesional kesehatan. Untuk masalah serius, konsultasikan dengan dokter ya"
                }
...
def check_security(user_input: str, user_id: str = "guest") -> dict:
    ...
    if not moderation["safe"]:
        return {
            ...
            "disclaimer": "",
            "metadata": {"reason": "harmful_content", "catagory": moderation["category"]}
        }
